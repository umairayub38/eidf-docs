...
| -------- | ------- | ------- |
|`/mnt/beegfs/<project_id>/shared`|/`safe_data`|Read-only access if required by IG, or read-write access, to data and other project files.|
|`/mnt/beegfs/<project_id>/users/<username>/outputs_<unique_id>`  |`/safe_outputs`  |Will be created at container startup as an empty directory. Intended for any outputs: logs, data, models.|
